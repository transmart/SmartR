...
window.smartRApp.controller('PPMIDemoController',
    ['$scope', 'smartRUtils', 'commonWorkflowService', function($scope, smartRUtils, commonWorkflowService) {
        ...
        var setVariantDBGeneList = function() {
            $.ajax({
                ...
                data: {
                    ...
                    path: '/genes/POST/',
                    filter_command: 'getfields!eq!hgnc_approved_symbol,chrom,genestart,genestop&hgnc_approved_symbol!eqa!' + $scope.pdmap.genes.join(','),
                },
                success: function(res) {
                    var data = JSON.parse(res);
                    var geneIdx = data.fields.indexOf('hgnc_approved_symbol');
                    var geneList = data.values.map(function(d) { return d[geneIdx]; });
                    $scope.variantDB.availableGenes = geneList;
                }
            });
        };
